...
from generators.utils.colors import label_color
...
def project_bbox_3D_to_2D(points_bbox_3D, rotation_vector, translation_vector, camera_matrix, append_centerpoint=True):
    ...
    points_bbox_2D, jacobian = cv2.projectPoints(points_bbox_3D,
                                                 np.float32(rotation_vector), np.float32(translation_vector),
                                                 camera_matrix,
                                                 None)
...
def draw_detections(image, boxes, scores, labels, rotations, translations, hand, class_to_bbox_3D, camera_matrix, color=None,
                    label_to_name=None, score_threshold=0.5, draw_bbox_2d=False, draw_name=False, draw_mano=False):
    ...
    for i in selection:
        ...
        if draw_bbox_2d:
            draw_box(image, boxes[i, :], color=(255, 0, 255))
        translation_vector = translations[i, :]
        points_bbox_2D = project_bbox_3D_to_2D(class_to_bbox_3D[labels[i].item()],
                                               rotations[i, :], translation_vector,
                                               camera_matrix, append_centerpoint=True)
        draw_bbox_8_2D(image, points_bbox_2D, color=c)
        if draw_name:
            if isinstance(label_to_name, dict):
                name = label_to_name[labels[i].item()] if label_to_name else labels[i]
            else:
        ...
        if draw_mano:
            draw_mano_coords(image, hand[i, :], camera_matrix, color=c)



def draw_annotations(image, annotations, class_to_bbox_3D, camera_matrix, color=(0, 255, 0), label_to_name=None,
                     draw_bbox_2d=False, draw_name=False, draw_mano=False):
    ...
    for i in range(annotations['bboxes'].shape[0]):
        ...
        points_bbox_2D = project_bbox_3D_to_2D(class_to_bbox_3D[annotations["labels"][i]],
                                               annotations['rotations'][i, :3], annotations['translations'][i, :],
                                               camera_matrix, append_centerpoint=True)
        ...
        if draw_mano:
            draw_mano_coords(image,
                             annotations['coords_3d'][i],
                             camera_matrix,
                             color=(color[0] * 0.7, color[1] * 0.7, color[2] * 0.7))

def draw_mano_coords(image, coords_3d, camera_matrix, color=(0, 255, 255)):
    """
    image: (256, 256, 3) ndarray
    coords_3d: (1, 21, 3) ndarray
    camera_matrix (3, 3) ndarray
    """
    # if input shape is (63,), reshape to (1, 21, 3)
    # if input shape is (21, 3) reshape to (1, 21, 3)
    coords_3d = np.reshape(coords_3d, (1, 21, 3))

    for i in range(coords_3d.shape[0]):
        coords_2d_plot = project_and_normalize(
            np.squeeze(coords_3d[i, :, :]),
            camera_matrix)
        cv2.polylines(image,
                      [coords_2d_plot.astype(int)[0:5, :]],  # palm and thumb
                      isClosed=False,
                      color=color,
                      thickness=2,
                      lineType=cv2.LINE_AA)
        cv2.polylines(image,
                      [np.concatenate((coords_2d_plot[0:1, :],coords_2d_plot[5:9, :])).astype(int)],  # index finger
                      isClosed=False,
                      color=color,
                      thickness=2,
                      lineType=cv2.LINE_AA)
        cv2.polylines(image,
                      [np.concatenate((coords_2d_plot[0:1, :],coords_2d_plot[9:13, :])).astype(int)],  # middle finger
                      isClosed=False,
                      color=color,
                      thickness=2,
                      lineType=cv2.LINE_AA)
        cv2.polylines(image,
                      [np.concatenate((coords_2d_plot[0:1, :],coords_2d_plot[13:17, :])).astype(int)],  # ring finger
                      isClosed=False,
                      color=color,
                      thickness=2,
                      lineType=cv2.LINE_AA)
        cv2.polylines(image,
                      [np.concatenate((coords_2d_plot[0:1, :],coords_2d_plot[17:21, :])).astype(int)],  # little finger
                      isClosed=False,
                      color=color,
                      thickness=2,
                      lineType=cv2.LINE_AA)
