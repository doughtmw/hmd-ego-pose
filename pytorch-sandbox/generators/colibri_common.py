...
def project_and_normalize(points_3d, cam_K):
    # project to 2d
    hom_points_3d = np.dot(cam_K, points_3d.transpose())  # (3, x)

    # Normalize homogeneous coordinates
    hom_points_2d = hom_points_3d / hom_points_3d[2, :]
    return hom_points_2d[:2, :].transpose()  # shape: (x,2)
...
class Generator(torch.utils.data.Dataset):
    ...
    def load_annotations_group(self, group):
        ...
        for annotations in annotations_group:
            ...
            assert (
                    'coords_3d' in annotations), '\'load_annotations\' should return a list of dictionaries that contain \'labels\', \'bboxes\', \'rotations\', \'translations\', \'translations_x_y_2D\', and \'coords_3d\'.'
    ...
    def augment_6DoF_image_and_annotations(self, img, annotations, mask, camera_matrix):
        ...
        # verts_3d_annos = np.zeros((778, 3))
        coords_3d_annos = np.zeros((21, 3))
        ...
        for i in range(num_annos):
            ...
            # verts_3d_annos = annotations["verts_3d"][i, :, :]
            coords_3d_annos = annotations["coords_3d"][i, :, :]
        ...
        augmented_img, \
        augmented_rotation_vector, augmented_translation_vector, \
        augmented_coords_3d_vector, \
        augmented_bbox, \
        still_valid_annos, is_valid_augmentation = self.augmentation_6DoF(
            img=img,
            mask=mask,
            rotation_matrix_annos=rotation_matrix_annos,
            translation_vector_annos=translation_vector_annos,
            coords_3d_annos=coords_3d_annos,
            angle=angle,
            scale=scale,
        ...
        if is_valid_augmentation:
            for i in range(num_annos):
                ...
                annotations["translations_x_y_2D"][i, :] = self.project_points_3D_to_2D(
                    ...
                    camera_matrix=camera_matrix)
                annotations["coords_3d"][i, :, :] = augmented_coords_3d_vector[i, :, :]
            ...
            annotations["coords_3d"] = np.delete(annotations["coords_3d"], annos_to_delete, axis=0)
            annotations["drill_tip_transform"] = np.delete(annotations["drill_tip_transform"], annos_to_delete, axis=0)
    ...
    def augmentation_6DoF(self, img, mask,
                          rotation_matrix_annos, translation_vector_annos,
                          # verts_3d_annos,
                          coords_3d_annos,
                          angle, scale, camera_matrix,
                          mask_values):
        ...
        augmented_coords_3d_vector_annos = np.zeros((num_annos, 21, 3))
        ...
        for i in range(num_annos):
            ...
            # rotate the hand vertices and coordinates with rotation matrix then scale
            tmp_transformation_matrix = np.concatenate([tmp_rotation_matrix, np.zeros((3, 1))], 1)
            hom_coords3d = np.concatenate([coords_3d_annos, np.ones((coords_3d_annos.shape[0], 1))], 1).transpose()  # shape: (4,21)
            augmented_coods_3d_annos = np.dot(tmp_transformation_matrix, hom_coords3d).transpose()  # shape: (21,3)
            augmented_coods_3d_annos[:, 2] /= scale
            ...
            augmented_coords_3d_vector_annos[i, :, :] = augmented_coods_3d_annos
            augmented_bbox_annos[i, :] = augmented_bbox
            still_valid_annos[i] = True

        return augmented_img, \
               augmented_rotation_vector_annos, augmented_translation_vector_annos, \
               augmented_coords_3d_vector_annos, augmented_bbox_annos, still_valid_annos, True
    ...
    def preprocess_group_entry(self, image, annotations, camera_matrix):
        ...
        # apply resizing to verts and coords
        # annotations['verts_3d'] *= image_scale
        annotations['coords_3d'] *= image_scale
    ...
    def preprocess_image(self, image):
        ...
        # cv2.imshow('pt-transforms', image)
        # cv2.imwrite('onnx-models/0000-pt-runtime-transforms.jpg', image)
