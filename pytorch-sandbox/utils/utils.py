...
# Take prediction tensor and bounding boxes as input and perform non-max 
# suppression on the data.
# def nms_processing(x, scores, boxes, threshold, iou_threshold):
#     scores_over_thresh = (scores > threshold)[:, :, 0]
#     out = []
#     for i in range(x.shape[0]):
#         if scores_over_thresh[i].sum() == 0:
...
#         # classification_per = classification[i, scores_over_thresh[i, :], ...].permute(1, 0)
#         classes_per = scores.max(dim=0)
#         transformed_anchors_per = boxes[i, scores_over_thresh[i, :], ...]
#         scores_per = scores[i, scores_over_thresh[i, :], ...]
#         # scores_, classes_ = classification_per.max(dim=0)
#         anchors_nms_idx = batched_nms(transformed_anchors_per, scores_per[:, 0], classes_per, iou_threshold=iou_threshold)

#         if anchors_nms_idx.shape[0] != 0:
#             classes_ = classes_per[anchors_nms_idx]
